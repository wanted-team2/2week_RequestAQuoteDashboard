...
const Header = ({companyName = 'A 가공 업체'} : HeaderProps) => {
    ...
    <S.HeaderWrapper>
      <S.HeaderRight>
          <HamburgerMobile className={'toggle-button'} onClick={handleToggleMenu}/>
          <S.Image>
            <img src={logo} alt="로고"/>
          </S.Image>
      </S.HeaderRight>
        <S.MenuWrapperMobile className={`menu-wrapper toggle-button ${slideToggle ? 'show' : 'hidden'}`} onClick={handleToggleMenu}>
          <S.HeaderMenu>
              <S.MenuItem>
                  <S.Image>
